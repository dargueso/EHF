...
def compute_EHF(tave,dates=None,thres_file=None,bsyear=None,beyear=None,month_starty=1,mask=None,method='NF13',nwindow=15,EHFaccl=False, season='yearly'):
  ...
  if (bsyear==None) or (beyear==None):
    sys.exit("ERROR: you didn't provide base period years to compute_EHF function, please revise")
  ...
  pct=calc_percentile(tave[(years>=bsyear) & (years<=beyear),:,:],nbyears,thres_file,method=method,nwindow=15)
  ...
  for ilat in range(nlat):
    for ilon in range(nlon):
      if mask[ilat,ilon] == 1:
        ...
        for t in range(ndays):
          if spell[t]!=0:
            heatwave_EHF_avg[t,ilat,ilon]=np.mean(EHF[t:t+spell[t],ilat,ilon])
            heatwave_EHF_peak[t,ilat,ilon]=np.max(EHF[t:t+spell[t],ilat,ilon])
            heatwave_EHF[t:t+spell[t],ilat,ilon]=EHF_exceed[t:t+spell[t],ilat,ilon]
  ...
  for yr in range(nyears):
    ...
    HWF[yr,:,:]=np.sum(spell_all[new_years==year,:,:],axis=0)*100./np.float(np.sum(new_years==year))
    HWN[yr,:,:]=np.sum(spell_all[new_years==year,:,:]!=0,axis=0)
    HWD[yr,:,:]=np.max(spell_all[new_years==year,:,:],axis=0)
    HWL[yr,:,:]=np.ma.sum(spell_all[new_years==year,:,:],axis=0)/HWN[yr,:,:]
    HWT[yr,:,:]=np.argmax(spell_all[new_years==year,:,:]!=0,axis=0)

    HWAt[yr,:,:]=np.ma.max(tave_peak_masked[new_years==year,:,:],axis=0)-const.tkelvin
    HWMt[yr,:,:]=np.ma.mean(tave_avg_masked[new_years==year,:,:],axis=0)-const.tkelvin

  HWT=np.ma.masked_equal(HWT,0.)
